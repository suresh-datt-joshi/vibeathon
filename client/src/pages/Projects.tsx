...
type DerivedProject = ProjectWithCounts & {
  progress: number;
  displayStatus: "completed" | "pending";
};
...
export default function Projects() {
  ...
  const projectsWithDerivedStatus = useMemo<DerivedProject[]>(() => {
    return projects.map((project) => {
      ...
      const displayStatus: "completed" | "pending" = isComplete
        ? "completed"
        : "pending";
  ...
  const filteredProjects = useMemo<DerivedProject[]>(() => {
    const term = searchTerm.trim().toLowerCase();
    if (!term) {
      return projectsWithDerivedStatus;
    }

    return projectsWithDerivedStatus.filter((project) => {
      const fields = [
        project.name,
        project.key,
        project.description ?? "",
  ...
  }, [projectsWithDerivedStatus, searchTerm]);
  ...
  const statusConfig = {
    completed: {
      ...
      icon: CheckCircle,
      className:
        "bg-[hsl(var(--lozenge-done-bg))] text-[hsl(var(--lozenge-done))] border-[hsl(var(--lozenge-done))]",
    },
    processing: {
      label: "In Progress",
      icon: Clock,
      className:
        "bg-[hsl(var(--lozenge-in-progress-bg))] text-[hsl(var(--lozenge-in-progress))] border-[hsl(var(--lozenge-in-progress))]",
    },
    pending: {
      label: "To Do",
      icon: AlertCircle,
            ...
            <Table>
              ...
              <TableBody>
                {filteredProjects.map((project) => {
                  const statusKey =
                    project.displayStatus as keyof typeof statusConfig;
